...
def get_obs(_rigid_body_pos, _rigid_body_rot, _rigid_body_vel, _rigid_body_ang_vel, _rigid_body_joints_indices,
            ...
            num_track_dev, reset_val,
            imit_motion_height, humanoid_height, include_global):
    # type: (Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, bool) -> Tensor
    ...
    reset_val = torch.unsqueeze(reset_val, dim=1)

    if include_global:
        o = torch.cat((root_global_pos, headset_global_pos, obs_sim, obs_user, obs_userheight, obs_simheight, reset_val), dim=-1)
    else:
        o = torch.cat((obs_sim, obs_user, obs_userheight, obs_simheight, reset_val), dim=-1)
    return o

#todo delete once not used anymore
@torch.jit.script
def get_obs_pre(_rigid_body_pos, _rigid_body_rot, _rigid_body_vel, _rigid_body_ang_vel, _rigid_body_joints_indices,
            dof_pos, dof_vel, feet_contact_forces, track_poses_acc, track_rots_acc, track_headset_index,
            num_track_dev,
            imit_motion_height, humanoid_height, include_global):
