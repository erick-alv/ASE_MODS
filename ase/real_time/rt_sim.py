...
from real_time.imitPoseState import ImitPoseStateAsyncSafe
...
def setup_envs(gym, sim, num_envs, num_per_row, spacing, humanoid_asset, num_bodies, start_pose):
    ...
    #uses the HMD (index0) as reference
...
async def main(stop_event, shared_imitState):
    while not (await shared_imitState.has_start_pose()):
        await asyncio.sleep(0.2)

    start_pose = await shared_imitState.get_start_pose()
...
def main_starter():
    ...
    imitState = ImitPoseStateAsyncSafe(1)
    # create function to insert content
    imitStateInsertFunc = lambda content: imitState.insert(content, transform_func=all_transforms, start_check_func=check_if_button_A_pressed)

    async def task_mngr(event_loop):
        stop_event = asyncio.Event()
        tasks = []
        tasks.append(
            event_loop.create_task(main(stop_event, imitState))
        )
        tasks.append(
