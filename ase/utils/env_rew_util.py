...
@torch.jit.script
def sum_as_questSim(
        dof_pos_sqdif_sum, dof_vel_sqdif_sum, rbj_pos_sqnorm_sum, rbj_vel_sqnorm_sum, feet_force_terms_sum,
...
def compute_reward(
    ...
    # type: (Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float) -> Tensor
    ...
    total_reward = sum_as_questSim(dof_pos_sqdif_sum, dof_vel_sqdif_sum, rbj_pos_sqnorm_sum, rbj_vel_sqnorm_sum,
                                   feet_force_terms_sum, w_dof_pos, w_dof_vel, w_pos, w_vel, w_force, k_dof_pos,
                                   k_dof_vel, k_pos, k_vel, k_force)
    return total_reward
...
def compute_reward_v1(
    ...
    dof_pos_sqdif_sum, dof_vel_sqdif_sum = estimate_dof_angle_point_dif(dof_pos, dof_pos_gt, dof_vel, dof_vel_gt)
    rbj_pos_sqnorm_sum, rbj_vel_sqnorm_sum = estimate_rigid_body_dif(rigid_body_pos, rigid_body_pos_gt, rigid_body_vel,
                                                                     rigid_body_vel_gt, rigid_body_joints_indices)
    feet_force_terms_sum = estimate_feet_force_term(feet_contact_forces, prev_feet_contact_forces)
...
#todo debug is that slight improvement at beginning is due to angle or due to penalty!!!
...
#extra term for not reaching same position as ground truth
@torch.jit.script
def compute_reward_v4(
        dof_pos, dof_pos_gt,
        dof_vel, dof_vel_gt,
        rigid_body_pos, rigid_body_pos_gt,
        rigid_body_vel, rigid_body_vel_gt,
    ...
    #todo once decided which angle dif is best use that one
    dof_pos_sqdif_sum, dof_vel_sqdif_sum = estimate_dof_angle_point_dif(dof_pos, dof_pos_gt, dof_vel, dof_vel_gt)
    rbj_pos_sqnorm_sum, rbj_vel_sqnorm_sum = estimate_rigid_body_dif(rigid_body_pos, rigid_body_pos_gt, rigid_body_vel,
                                                                     rigid_body_vel_gt, rigid_body_joints_indices)
    feet_force_terms_sum = estimate_feet_force_term(feet_contact_forces, prev_feet_contact_forces)
    ...
    total_reward = r_dof_pos + r_dof_vel + r_pos + r_vel + r_force + r_reach_pos
...
# extra term for moving feet when there is no movement
@torch.jit.script
